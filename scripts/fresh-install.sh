...
HOST_FOLDER="$1"
if [ ! -e "$HOME/infra/$HOST_FOLDER" ]; then
...
sudo nix --experimental-features "nix-command flakes" run github:nix-community/disko -- --mode disko --flake "$HOME/infra/$HOST_FOLDER"
...
sudo cp -r /mnt/etc/nixos/hardware-configuration.nix "$HOME/infra/$HOST_FOLDER/"

echo "Disk disko complete. Now installing NixOS to disk..."
sudo nixos-install --root /mnt --no-root-passwd --flake "$HOME/infra/$HOST_FOLDER"
